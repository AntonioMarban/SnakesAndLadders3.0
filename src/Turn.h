...
class Turn {
    ...
    public:
        Turn() = default;
        Turn(const int& turnNumber, const int& playerNumber, const int& playerStartingPosition, const int& rolled, const char& tileType, const int& playerFinalPosition) : turnNumber(turnNumber), playerNumber(playerNumber), playerStartingPosition(playerStartingPosition), rolled(rolled), tileType(tileType), playerFinalPosition(playerFinalPosition) {}
        ~Turn();
        int getTurnNumber();
        int getPlayerNumber();
